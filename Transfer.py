    ...
    def SendTransfer(self):
        ...
        try:
            ...
            try:
                ...
                if amount <= 0:
                    self.message('Invalid Amount', 'Amount must be positive')
                    return
            except ValueError:
                self.message('Invalid Amount', 'Please enter a valid number')
                return

            # Check sender account with error handling
            cur.execute("SELECT BAL FROM NEWBANK WHERE USERNAME = ?", (sender_username,))
            sender_data = cur.fetchone()
            
            if sender_data is None:
                self.message('Error', 'Sender account not found')
                return
                
            try:
                sender_balance = float(sender_data[0]) if sender_data[0] is not None else 0.0
            except ValueError:
                self.message('Data Error', f'Sender balance is invalid: {sender_data[0]}')
                return

            # Check receiver account with error handling
            cur.execute("SELECT BAL FROM NEWBANK WHERE USERNAME = ?", (receiver_username,))
            receiver_data = cur.fetchone()
            
            if receiver_data is None:
                self.message('Error', 'Receiver account not found')
                return
                
            try:
                receiver_balance = float(receiver_data[0]) if receiver_data[0] is not None else 0.0
            except ValueError:
                self.message('Data Error', f'Receiver balance is invalid: {receiver_data[0]}')
                return

            # Validate sender balance
            if sender_balance < amount:
                self.message('Error', 'Insufficient funds')
                return
            ...
            cur.execute("UPDATE NEWBANK SET BAL = ? WHERE USERNAME = ?", 
                    (str(new_sender_bal), sender_username))  # Convert to string if needed
            cur.execute("UPDATE NEWBANK SET BAL = ? WHERE USERNAME = ?",
                    (str(new_receiver_bal), receiver_username))
            ...
            """, (
                sender_username, receiver_username, selected_type, amount,
                sender_balance, new_sender_bal,
                receiver_balance, new_receiver_bal
            ))

            conn.commit()
            self.message('Success', 'Transfer completed')

        except Exception as e:
            conn.rollback()
            self.message('Error', f'Transfer failed: {str(e)}')
        finally:
            conn.close()
